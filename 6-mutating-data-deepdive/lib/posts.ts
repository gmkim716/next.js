...
export async function getPosts(maxNumber: number | undefined = undefined) {
  let limitClause = '';
...
export async function storePost(post: {imageUrl: string, title: string, content: string, userId: number}) {
  const stmt = db.prepare(`
